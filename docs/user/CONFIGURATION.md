...
This document describes all configuration options and settings available in the Cloud Voice Assistants custom integration.
...
### `cloud_voice_assistants.example_service`
...
```yaml
service: cloud_voice_assistants.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: cloud_voice_assistants.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Cloud Voice Assistants Alert
...
- [GitHub Issues](https://github.com/SashaBusinaro/ha-cloud-voice-assistants/issues) - Report problems
