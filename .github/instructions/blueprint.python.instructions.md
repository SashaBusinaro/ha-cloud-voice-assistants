...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `CloudVoiceAssistants`
...
- Inherit from both platform entity and `CloudVoiceAssistantsEntity` (order matters)
...
from custom_components.cloud_voice_assistants.sensor import async_setup_entry
...
**Config entry data:** `entry_data: CloudVoiceAssistantsData = hass.data[DOMAIN][entry.entry_id]`
