...
- **Repository:** SashaBusinaro/ha-cloud-voice-assistants

**Key directories:**

- `custom_components/cloud_voice_assistants/` - Main integration code
...
- Integration logs: `custom_components.cloud_voice_assistants: debug` in `config/configuration.yaml`
...
- Use the domain `cloud_voice_assistants` for all DOMAIN references
- Prefix all integration-specific classes with `CloudVoiceAssistants`
- Use "Cloud Voice Assistants" as the display title
...
- Inherit from platform base + `CloudVoiceAssistantsEntity`
...
- `tests/` mirrors `custom_components/cloud_voice_assistants/` structure
